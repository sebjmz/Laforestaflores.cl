...
function openB2BFlow() {
    ...
    if (e) {
        ...
        "function" == typeof window.stopAutoPlay && window.stopAutoPlay();
    }
}
...
function selectB2BSegment(e) {
    document.getElementById("b2b-selected-segment").value = e;
    ...
    setTimeout(() => {
        ...
        setTimeout(() => {
            ...
            setTimeout(() => { r.classList.remove("opacity-0"); }, 400);
        }, 10);
    }, 300);
}
...
async function sendB2BConsultation(e) {
    ...
    let t = document.getElementById("b2b-submit-btn"), a = t.innerHTML;
...
let cart = [],
    ...
    selectedTimeSlot = "",
    isExpressDelivery = !1,
    b2bTurnstileToken = null,
    gardenTurnstileToken = null;
...
try {
    if ("undefined" != typeof MercadoPago) {
...
const zoneData = [
    ...
    { id: 5, radius: 20000, color: "#c5a059", fillOpacity: .03, name: "Extendida+" }
],
atelierCenter = [-32.97074205954955, -71.5431332198481];
...
function initCoverageMap() {
    ...
    [...zoneData].reverse().forEach(e => {
        let t = L.circle(atelierCenter, { radius: e.radius, color: e.color, weight: 1, fillColor: e.color, fillOpacity: e.fillOpacity, interactive: !1 }).addTo(coverageMap);
        circleLayers[e.id] = t;
    });
    ...
    L.marker(atelierCenter, { icon: t }).addTo(coverageMap)
        .bindTooltip("Atelier", { permanent: !0, direction: "right", className: "atelier-tooltip" })
        .openTooltip();
    mapInitialized = !0;
}
...
function resetZone(e) {
    ...
    let t = zoneData.find(t => t.id === e);
    circleLayers[e].setStyle({ fillOpacity: t.fillOpacity, weight: 1, color: t.color });
    [...zoneData].reverse().forEach(e => {
        if (circleLayers[e.id]) circleLayers[e.id].bringToFront();
    });
}
...
function addToCart(e, t, a, r) {
    try {
        let o = cart.find(t => t.id === e);
        o ? o.qty++ : cart.push({ id: e, name: t, price: a, img: r, qty: 1 });
        localStorage.setItem("laforesta_cart", JSON.stringify(cart));
        "function" == typeof updateCartUI && updateCartUI();
...
function updateQty(e, t) {
    ...
    if (a) {
        a.qty += t;
        a.qty <= 0 ? removeFromCart(e) : updateCartUI();
    }
}

function updateCartUI() {
    try {
        let e = cart.some(e => e.id < 100 || (e.id >= 201 && e.id <= 204));
        if (!e) cart = [];
        localStorage.setItem("laforesta_cart", JSON.stringify(cart));
        ...
        let s = cart.map(e => {
            ...
            <div class="flex space-x-4 items-center">
                <div class="w-24 h-24 flex-shrink-0 bg-white overflow-hidden relative flex items-center justify-center border border-zinc-200 rounded-md shadow-sm">
                    <img src="img/${t}" class="w-full h-full object-cover relative z-10" alt="${e.name}">
                </div>
                <div class="flex-grow">
                    <h3 class="font-serif text-lg italic mb-1 text-[#0a1f1c]">${e.name}</h3>
                    <p class="text-[#0a1f1c] text-sm font-bold">$${e.price.toLocaleString()}</p>
                    <div class="flex items-center space-x-6 mt-3">
                        <div class="flex items-center space-x-3 border border-zinc-200 px-2 py-1 bg-white rounded">
                            <button onclick="updateQty(${e.id}, -1)" aria-label="Disminuir cantidad" class="hover:text-[#c5a059] transition-colors">-</button>
                            <span class="text-xs font-bold w-4 text-center">${e.qty}</span>
                            <button onclick="updateQty(${e.id}, 1)" aria-label="Aumentar cantidad" class="hover:text-[#c5a059] transition-colors">+</button>
                        </div>
                        <button onclick="removeFromCart(${e.id})" class="text-[9px] uppercase tracking-widest opacity-70 hover:opacity-100 transition-opacity">Eliminar</button>
                    </div>
                </div>
            </div>
            `;
        ...
        if (void 0 !== upsellsCatalog) {
            let l = cart.map(e => e.id),
                c = upsellsCatalog.filter(e => !l.includes(e.id)),
                d = [13, 14, 15, 16, 17, 201, 202, 203, 204],
                p = cart.some(e => d.includes(e.id));
                    ...
                    <div class="mt-8 border-t border-zinc-100 pt-6">
                        ...
                        <div id="upsell-slider" class="flex overflow-x-auto space-x-4 pb-4 hide-scroll w-full cursor-grab active:cursor-grabbing">
                            ${c.map(e => {
                                let t = e.img.split("/").pop();
                                return `
                                <div class="w-[110px] flex-shrink-0 group cursor-pointer" role="button" tabindex="0" aria-label="Agregar ${e.name} por $${e.price}" onclick="addToCart(${e.id}, '${e.name}', ${e.price}, 'img/${t}')">
                                    <div class="w-[110px] h-[110px] bg-white border border-zinc-200 mb-2 flex items-center justify-center relative overflow-hidden transition-all group-hover:border-[#c5a059] rounded-md shadow-sm p-2">
                                        ...
                                        <img src="img/${t}" loading="lazy" class="w-full h-full object-contain relative z-10 transition-transform duration-500 group-hover:scale-105" alt="${e.name}">
                                    </div>
                                    <h6 class="text-[9px] uppercase tracking-widest font-bold leading-tight text-[#0a1f1c] mb-1 line-clamp-2">${e.name}</h6>
                                    <p class="text-[#0a1f1c] text-[10px] font-bold">+$${e.price.toLocaleString()}</p>
                                </div>
                                `;
                            }).join("")}
                        </div>
        ...
        if (m) m.addEventListener("wheel", e => {
            if (0 !== e.deltaY) {
                e.preventDefault();
        ...
        "function" == typeof actualizarTotalConDespacho && actualizarTotalConDespacho();
        
        let u = cart.reduce((e, t) => e + t.qty, 0);
        if (r) r.innerText = u;
        
        let g = document.getElementById("mobile-sticky-cart");
        if (g) {
            let $ = document.getElementById("sticky-cart-count"),
                y = document.getElementById("sticky-cart-total");
            if ($) $.innerText = u;
            if (y && a) y.innerText = a.innerText;
            cart.length > 0 ? g.classList.remove("translate-y-full") : g.classList.add("translate-y-full");
        }
    } catch (f) {
...
function restaurarProgresoCheckout() {
    ...
    if (e) {
        ["sender-name", "buyer-email", "receiver-name", "address", "pickup-name", "card-message", "receiver-phone"].forEach(t => {
            let a = document.getElementById(t);
            if (a) a.value = e[t.replace(/-([a-z])/g, e => e[1].toUpperCase())] || "";
        });
        
        if ("true" === localStorage.getItem("laforesta_checkout_abierto")) {
...
function openCheckout() {
    ...
    if ("step-time" === r) renderHorariosInteligentes();
    
    let o = cart.reduce((e, t) => e + t.price * t.qty, 0);
    trackGA4("begin_checkout", {
        currency: "CLP",
        value: o,
        items: cart.map(e => ({ item_id: String(e.id), item_name: e.name, price: e.price, quantity: e.qty, item_category: "Ramos Florales" }))
    });
}
...
function goToStep(e) {
    let t = "number" == typeof e ? `step-${e}` : e;
...
function selectOption(e, t) {
    ...
    if (a) {
        a.querySelectorAll(".step-option").forEach(e => {
            e.style.borderColor = "rgba(10, 31, 28, 0.15)";
...
function regresarDesdeFecha() {
    "envio" === selectedLogistics ? goToStep("step-direccion") : goToStep("step-quien-retira");
}
...
function renderCalendar() {
    ...
    t.innerText = `${["Enero", "Febrero", "Marzo", "Abril", "Mayo", "Junio", "Julio", "Agosto", "Septiembre", "Octubre", "Noviembre", "Diciembre"][o]} ${r}`;
    ...
    document.getElementById("cal-prev").style.visibility = r === i && o === n ? "hidden" : "visible";
    document.getElementById("cal-next").style.visibility = r === p && o === d ? "hidden" : "visible";
    ...
    for (let $ = 1; $ <= u; $++) {
        ...
        if (y.getTime() <= f.getTime()) {
            e.innerHTML += `<div class="py-3 text-zinc-300 font-serif text-sm cursor-not-allowed">${$}</div>`;
        } else {
            let b = `${r}-${String(o + 1).padStart(2, "0")}-${String($).padStart(2, "0")}`;
            e.innerHTML += `<button onclick="seleccionarDiaCalendario('${b}')" aria-label="Seleccionar el ${$} de este mes" class="py-3 font-serif text-sm hover:text-gold-leaf transition font-bold text-[#0a1f1c]">${$}</button>`;
        }
    }
}
...
function renderHorariosInteligentes() {
    ...
    let a = new Date(new Date().toLocaleString("en-US", { timeZone: "America/Santiago" })),
        ...
        s = "hoy" === selectedDate || selectedDate === i,
        l = ["Enero", "Febrero", "Marzo", "Abril", "Mayo", "Junio", "Julio", "Agosto", "Septiembre", "Octubre", "Noviembre", "Diciembre"];
    
    if (s) {
        if (t) t.innerText = `Para Hoy: ${a.getDate()} de ${l[a.getMonth()]} de ${a.getFullYear()}`;
    } else {
        let c = selectedDate.split("-");
        if (3 === c.length) {
            let d = c[0], p = parseInt(c[1], 10) - 1, m = parseInt(c[2], 10);
            if (t) t.innerText = `Para el ${m} de ${l[p]} de ${d}`;
        } else {
    ...
    if (s) {
        ...
        if ("envio" === selectedLogistics && n < 18) $ = generarHorasExpress(n);
        ...
        if ("" !== $) u += `<div class="border-t border-[#c5a059]/30 pt-6 mt-2"><span class="text-[9px] font-bold uppercase tracking-widest text-[#c5a059] block mb-3 text-left">Servicio Express (1 hr)</span><div class="flex flex-col gap-2">${$}</div></div>`;
...
function regresarDesdeTime() {
    "hoy" === selectedDate ? goToStep("step-fecha") : goToStep("step-calendario");
}
...
function actualizarTotalConDespacho() {
    let e = cart.reduce((e, t) => e + t.price * t.qty, 0),
        t = parseInt(shippingCost || 0, 10),
        a = isExpressDelivery ? Math.round(1.5 * t) : t,
        r = 0;
    ...
    if (u) {
        if (isExpressDelivery && "envio" === selectedLogistics) {
...
function nextGardenStep(e) {
    document.querySelectorAll(".garden-step").forEach(e => {
...
function sendGardenConsultation() {
    ...
    let r = `*SOLICITUD ASESORÍA PAISAJISMO - LA FORESTA*\n\n*CLIENTE:* ${e}\n*TIPO DE PROYECTO:* ${selectedGardenSpace}\n*FONO CONTACTO:* ${a}\n\n*DETALLES:*\n"${t}"`,
        o = `https://wa.me/56994783520?text=${encodeURIComponent(r)}`;
    window.open(o, "_blank");
    closeGardenFlow();
}

function obtenerPayloadOrden() {
    let e = document.getElementById("sender-name")?.value || "No especificado",
        t = document.getElementById("buyer-email")?.value.trim() || "",
        a = document.getElementById("receiver-name")?.value || "No especificado",
        r = document.getElementById("card-message")?.value || "",
        o = document.getElementById("receiver-phone")?.value || "",
        n = "envio" === selectedLogistics ? `Envío a Domicilio (${selectedZoneName})\n• *DIRECCIÓN:* ${document.getElementById("address")?.value || ""}` : `Retiro en Atelier Reñaca\n• *RETIRA:* ${document.getElementById("pickup-name")?.value || ""}`;
    
    if (isExpressDelivery && "envio" === selectedLogistics) n = "[SERVICIO EXPRESS] " + n;
    
    let i = selectedTimeSlot || "No especificado",
        s = "hoy" === selectedDate ? "Hoy" : selectedDate,
        l = cart.reduce((e, t) => e + t.price * t.qty, 0),
        c = parseInt(shippingCost || 0),
        d = isExpressDelivery ? Math.round(1.5 * c) : c,
        p = Math.max(0, d - (l >= 69990 ? Math.min(d, 7250) : 0)),
        m = l + p;
...
async function iniciarMercadoPago() {
    if (0 === cart.length) return alert("El atelier está vacío.");
    ...
    let e = obtenerPayloadOrden();
    let t = "LF-" + Date.now();
    let a = e.metadata.sender_name.trim().split(" ");
    let r = a[0] || "Cliente";
    let o = a.slice(1).join(" ") || "La Foresta";
    
    let n = [];
    if ((window.descuentoPuntos && window.descuentoPuntos > 0) || (window.tierDiscount && window.tierDiscount > 0)) {
        let itemsTotalClp = e.totalCLP - (e.metadata.valor_envio || 0);
        n.push({
            id: "PEDIDO_DESC",
            title: "Pedido La Foresta (Descuentos Aplicados)",
            description: "Ramos y beneficios del Club",
            quantity: 1,
            unit_price: itemsTotalClp,
            currency_id: "CLP"
        });
    } else {
        n = cart.map(item => ({
    ...
    if (e.metadata.valor_envio > 0) {
        n.push({
            id: "ENVIO",
            title: `Envío a Domicilio - ${selectedZoneName}`,
            description: "Servicio de logística",
            quantity: 1,
            unit_price: e.metadata.valor_envio,
            currency_id: "CLP"
        });
    }
    ...
    let i = {
        ...
        items: n,
        payer: { name: r, surname: o, email: e.metadata.comprador_email },
        back_urls: {
    ...
    try {
        ...
        let l = await s.json();
        if (l.init_point) window.location.href = l.init_point;
        else throw Error("No se pudo generar el link de pago.");
    } catch (c) {
        console.error(c);
...
function iniciarPayPal() {
    ...
    let e = document.getElementById("paypal-container");
    e.classList.remove("hidden");
    e.style.display = "block";
    e.innerHTML = "";
    
    let t = obtenerPayloadOrden();
    ...
    paypal.Buttons({
        ...
        createOrder: async function () {
            let e = await fetch("https://old-brook-bf01.sebjmz.workers.dev/paypal/create-order", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ totalCLP: t.totalCLP, metadata: t.metadata })
            });
            let a = await e.json();
            if (a.id) return a.id;
            throw Error("Error generando orden de PayPal");
        },
        onApprove: async function (e, a) {
            ...
            try {
                let o = await fetch("https://old-brook-bf01.sebjmz.workers.dev/paypal/capture-order", {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({ orderID: e.orderID, metadata: t.metadata })
                });
                let n = await o.json();
...
if (grid) {
    grid.innerHTML = catalog.map(e => {
        let t = getLuxuryBadge(e.id),
            ...
            r = 3 === e.id || 5 === e.id ? `<div class="urgency-tag">Últimas 2 unidades</div>` : "";
        return `
        <div class="product-card group flex flex-col h-full">
            ...
            <div class="mt-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-3">
                <span class="font-serif text-base md:text-xl text-[#0a1f1c]">$${e.price.toLocaleString()}</span>
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    let e = new URLSearchParams(window.location.search),
        t = e.get("payment_result"),
        a = e.get("status") || e.get("collection_status");
    
    if ("success" === t && "approved" === a) {
        let r = JSON.parse(localStorage.getItem("laforesta_cart") || "[]"),
            o = parseInt(localStorage.getItem("laforesta_shippingCost") || "0") + r.reduce((e, t) => e + t.price * t.qty, 0),
            n = "LF-" + Date.now();
        trackGA4("purchase", {
            transaction_id: n,
            currency: "CLP",
            value: o,
            items: r.map(e => ({ item_id: String(e.id), item_name: e.name, price: e.price, quantity: e.qty, item_category: "Ramos Florales" }))
        });
    ...
    } else if (("failure" === t || "pending" === t || "rejected" === a || "cancelled" === a)) {
        localStorage.removeItem("pending_laforesta_order");
        alert("El pago no fue acreditado o fue cancelado. Intente nuevamente.");
        window.history.replaceState({}, document.title, window.location.pathname);
    }
    
    let i = document.getElementById("year-footer");
    if (i) i.innerText = new Date().getFullYear();
    
    let s = document.getElementById("garden-flow");
    if (s) s.style.transform = "translateY(100%)";
    
    updateCartUI();
    restaurarProgresoCheckout();
    
    if ("open_cart" === e.get("action")) {
        setTimeout(() => {
            let e = document.getElementById("atelier-bag");
            if (e && !e.classList.contains("open")) toggleCart();
        }, 150);
    ...
    let l = document.querySelectorAll(".collage-page");
    l.forEach(e => {
        e.addEventListener("mouseenter", function () {
            if (window.innerWidth > 1024) {
                l.forEach(e => e.classList.remove("active-page"));
    ...
    if (window.innerWidth <= 1024) {
        let c = { root: null, rootMargin: "-35% 0px -35% 0px", threshold: 0 },
    ...
    if (p.length > 0) {
        let m = 0, u = p.length, g = document.getElementById("hero-dots"), $, y = !1, f = !1;
        ...
        function b(e, t) {
            ...
            r.style.zIndex = "20";
            a.style.zIndex = "10";
            "next" === t ? r.style.transform = "translate3d(100%, 0, 0)" : r.style.transform = "translate3d(-100%, 0, 0)";
            requestAnimationFrame(() => {
                requestAnimationFrame(() => {
                    let o = "0.6s", n = "cubic-bezier(0.16, 1, 0.3, 1)";
                    r.style.transition = `transform ${o} ${n}`;
                    a.style.transition = `transform ${o} ${n}`;
                    r.style.transform = "translate3d(0%, 0, 0)";
                    "next" === t ? a.style.transform = "translate3d(-100%, 0, 0)" : a.style.transform = "translate3d(100%, 0, 0)";
                    m = e;
                    v();
                    h();
                    setTimeout(() => { f = !1; }, 600);
        ...
        function v() {
            if (g) {
                Array.from(g.children).forEach((e, t) => {
                    let a = e.querySelector(".dot-visual") || e;
                    if (t === m) {
                        a.classList.remove("bg-transparent");
                        a.classList.add("bg-[#c5a059]", "scale-125");
                    } else {
                        a.classList.remove("bg-[#c5a059]", "scale-125");
        ...
        function h() {
            p.forEach((e, t) => {
                let a = e.querySelectorAll(".slide-anim");
                if (t === m) {
                    setTimeout(() => {
                        a.forEach(e => {
                            e.classList.remove("opacity-0", "translate-y-4", "translate-y-8");
                    ...
                    a.forEach(e => {
                        ...
                        "H2" === e.tagName ? e.classList.add("opacity-0", "translate-y-8") : e.classList.add("opacity-0", "translate-y-4");
                    });
                }
            });
        }
        ...
        p.forEach((e, t) => {
            ...
            if (g) {
                ...
                a.setAttribute("aria-label", "Ver diapositiva " + (t + 1));
                a.className = "w-6 h-6 md:w-7 md:h-7 flex items-center justify-center rounded-full transition-all duration-300";
                a.innerHTML = `<span class="dot-visual block w-2 h-2 md:w-2.5 md:h-2.5 rounded-full border border-[#c5a059] transition-all duration-300 ${0 === t ? "bg-[#c5a059] scale-125" : "bg-transparent"}"></span>`;
                a.onclick = () => {
                    if (f || m === t) return;
                    "function" == typeof window.stopAutoPlay && window.stopAutoPlay();
                    let e = t > m ? "next" : "prev";
                ...
                a.addEventListener("mouseenter", () => {
                    let e = a.querySelector(".dot-visual");
                    if (e && t !== m) e.style.backgroundColor = "rgba(197,160,89,0.5)";
                });
                a.addEventListener("mouseleave", () => {
                    let e = a.querySelector(".dot-visual");
                    if (e && t !== m) e.style.backgroundColor = "";
                });
                g.appendChild(a);
            }
        });
        ...
        window.nextHeroSlide = function (e = !1) {
            if (e && "function" == typeof window.stopAutoPlay) window.stopAutoPlay();
            if (f) return;
            let t = (m + 1) % u;
            b(t, "next");
        };
        window.prevHeroSlide = function (e = !1) {
            if (e && "function" == typeof window.stopAutoPlay) window.stopAutoPlay();
            if (f) return;
            let t = (m - 1 + u) % u;
            b(t, "prev");
        ...
        h();
        $ = setInterval(() => { if (!y) nextHeroSlide(!1); }, 8000);
        
        let _ = document.getElementById("slider-wrapper");
        if (_) {
            let x = 0, E = 0;
            _.addEventListener("touchstart", e => { x = e.changedTouches[0].screenX; }, { passive: !0 });
            _.addEventListener("touchend", e => {
                E = e.changedTouches[0].screenX;
                if (E < x - 50) window.nextHeroSlide(!0);
                if (E > x + 50) window.prevHeroSlide(!0);
            }, { passive: !0 });
        }
    }
});
